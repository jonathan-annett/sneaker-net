...
if exist %RFLAG% if exist %STORE% if exist %SRC% goto usb_ready
...
:usb_ready
echo Checking USB (%USB%:) against %SRC%

rem XCOPY  %SRC%* %STORE% /D /S /E /Y 
ROBOCOPY %SRC% %STORE% /MIR /XO /XA:SH /XD *$RECYCLE.BIN*
